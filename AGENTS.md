...
- **测试基建断裂(2026-09 发现)**:tsx 对**方法装饰器**发射 TC39 stage-3 签名(`__decorateElement`),而框架装饰器(`D.linkUtil` 等)按 legacy 签名写(`target, property, descriptor`)——两个 cocos 包共 6 个存量测试失败(`link util failed, [object Object] in Function is not a valid function`,EngineLinker/UISystem 的 `@D.linkUtil` 处崩溃;类装饰器 `@D.store`/`@D.system` 不受影响,故 k-ts-framework 16 用例可过)。`tsconfig` 显式 `experimentalDecorators` 无效(tsx 转换层不沿 extends 链也不认显式声明)。修法:装饰器实现双签名兼容(检测 tc39 `context` 对象)或锁定 tsx 转换行为。
