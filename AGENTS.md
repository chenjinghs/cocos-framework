...
- **装饰器双签名兼容(2026-09 修复)**:tsx 对**方法/字段装饰器**发射 TC39 stage-3 签名(`__decorateElement`),与 legacy 签名(`target, property, descriptor` / `target, propertyKey`)不兼容——类声明/表达式一律如此,类装饰器仅因签名位置兼容而幸存。已修复:`D.linkUtil`(`k-ts-framework/src/misc/UtilLinker.ts`,stage-3 判定 `typeof property === "object" && property !== null && typeof property.kind === "string"`,ctor 经 `addInitializer` 惰性捕获,`getCtor()` 兜底断言)与 `@onPrefabEvent`/`@bind`/`@bindChildren`(`k-ui-framework-cocos/src/DecoratorExtension.ts` 的 `decorateMember` 助手,判定 `typeof ctx.name === "string" || typeof ctx.name === "symbol"`,stage-3 下注册延迟到首个实例构造、闭包 once 防重复;返回 undefined=保留原值)。stage-3 的 `context` 对象上 `kind`/`name` 任一可判(两处实码分别用 kind:string 与 name:string|symbol,语义等价)。生产链路(tsc/Creator 构建)走 legacy 分支语义不变。新增装饰器若需在测试中经过 tsx,必须同样做双签名兼容。
