...
import type { IEngine, IPatchApplyResult } from "../src/Define";
import type { IFileInfo, IManifestInfo } from "patch-common";

// 用真实文件和 C# File.Move 的“不覆盖”语义执行生产 Patcher，替换的只有 Unity 桥接。
...
const csArray = (values: string[]) => ({ Length: values.length, get_Item: (i: number) => values[i] });
const exists = (path: string, directory: boolean) => nodeFS.existsSync(path) && nodeFS.statSync(path).isDirectory() === directory;
Object.assign(globalThis, {
    CS: {
        UnityEngine: {
            GameObject: {},
            SystemLanguage: new Proxy({}, { get: (_target, key) => key }),
            Application: { systemLanguage: "ChineseSimplified" },
        },
        KPlayerPrefs: { GetString: (_key: string, fallback: string) => fallback },
        BufferUtil: { ToBuffer: (buffer: Buffer) => Uint8Array.from(buffer).buffer },
        System: { IO: {
            File: {
                Exists: (path: string) => exists(path, false),
                ReadAllText: (path: string) => nodeFS.readFileSync(path, "utf8"),
                ReadAllBytes: (path: string) => nodeFS.readFileSync(path),
                WriteAllText: (path: string, text: string) => io("write", path, undefined, () => nodeFS.writeFileSync(path, text)),
                Copy: (source: string, target: string) => {
                    assert.equal(nodePath.basename(source), "patcher.js", "整理阶段不应复制资源内容");
                    nodeFS.copyFileSync(source, target);
                },
                Move: (source: string, target: string) => io("move", source, target, () => {
                    assert.equal(nodeFS.existsSync(target), false, `File.Move target already exists: ${target}`);
                    nodeFS.renameSync(source, target);
                }),
                Delete: (path: string) => io("delete", path, undefined, () => nodeFS.rmSync(path, { force: true })),
            },
            Directory: {
                Exists: (path: string) => exists(path, true),
                CreateDirectory: (path: string) => nodeFS.mkdirSync(path, { recursive: true }),
                Delete: (path: string) => nodeFS.rmSync(path, { recursive: true }),
                GetFiles: (path: string) => csArray(nodeFS.readdirSync(path).map((name) => nodePath.join(path, name)).filter((file) => exists(file, false))),
            },
            Path: {
                Combine: (...paths: string[]) => nodePath.join(...paths),
                GetDirectoryName: nodePath.dirname,
                GetFileName: nodePath.basename,
                GetFullPath: nodePath.resolve,
            },
        } },
    },
});
...
class Fixture {
    ...
    engine(overrides: Partial<IEngine> = {}): IEngine {
        return {
            entryUrl: "https://patch.invalid", downloadPath: this.downloadPath, applyPath: this.applyPath,
            apply: true, localResVersion: 1, maxRetryCount: 2, retryIntervalMS: 1, branchVersion: 1,
            onApplyCurrentManifest: (result) => { this.applied.push(result); },
            onResetToBuiltinResources: () => {}, onCheckNetwork: async () => false,
            onTrackPoint: () => {}, onDownloadProgressChanged: () => {},
            onOrganizeProgressChanged: (current) => { this.progress.push(current); },
            onComplete: () => {}, onError: async (tag, message) => { this.errors.push(`${tag}: ${message}`); },
            onNewAppNeedDownload: async () => {}, onRestartPatcher: () => {}, onPatchCanDownload: async () => true,
            onUnzipFile: async () => {}, onFetchRemoteText: async () => { throw new Error("unexpected network request"); },
            onDownloadFile: async () => { throw new Error("unexpected download"); },
            getOSType: () => "android", getDistroName: () => "", getRegion: () => "cn",
            setPreRelease: () => {}, setIsReviewMode: () => {}, onFetchNewPatchInfoResult: () => {},
            setNewEntryUrl: () => {}, setPatchInfo: () => {}, ...overrides,
        };
    }
...
async function withFixture(action: (fixture: Fixture) => Promise<void>, count = 3) {
    ...
    finally { observeIO = undefined; beforeIO = undefined; fixture.remove(); }
}
...
test("批量移动真实文件，主线程定时器持续运行，进度按文件数单调增加", async () => {
    await withFixture(async (f) => {
        ...
        try { await f.run(); } finally { clearInterval(timer); }
        f.assertApplied();
...
test("每次移动、备份替换、manifest 写入之后退出，重启仍能恢复完整资源", async () => {
    await withFixture(async (f) => {
        ...
        observeIO = () => { snapshots.push(f.snapshot()); };
        await f.run();
        observeIO = undefined;
...
test("common 和语言包依次下载整理，全部完成后才提交 PatchFinish", async () => {
    await withFixture(async (f) => {
        ...
        let finishVersion = 0;
        await startPatcher(f.engine({
            localCheckMode: true,
            onCheckNetwork: async () => true,
            onFetchRemoteText: async () => JSON.stringify({
                oldestVersion: 1, currentVersion: 2,
                channels: {
            ...
            onDownloadFile: async (url, target) => { downloads.push(url); f.write(target, "zip fixture"); },
        ...
        }), [EGameLanguage.RU_RU], EGameLanguage.RU_RU);
        f.assertApplied();
...
test("整理进度不显示下载速度，下一渠道重新开始下载计时", async () => {
    await withFixture(async (f) => {
        const { UpdateUI } = await import("../src/UpdateUI");
        const ui = new UpdateUI(f.engine());
        const slider = { value: 0, DOKill: () => {} };
        const text = { text: "" };
        Object.assign(ui, { progressSlider: slider, progressText: text });
        ui.setProgress(1n, 1n, true);
        ui.setOrganizeProgress(2, 8);
        assert.equal(slider.value, 0.25);
        assert.match(text.text, /2\/8/);
        assert.equal(text.text.includes("/s"), false);
        ui.setOrganizeProgress(8, 8);
        assert.match(text.text, /8\/8/);
        ui.setProgress(1n, 10n, true);
        ui.setProgress(2n, 10n, true);
        assert.match(text.text, /\/s/);
...
for (const invalidChannels of [false, true]) {
    test(`资源版本已最新时仍能自更新脚本，旧协议=${invalidChannels}`, async () => {
        await withFixture(async (f) => {
            ...
            let restarted = false;
            await f.run({
                localCheckMode: true, onCheckNetwork: async () => true,
                onFetchRemoteText: async () => JSON.stringify({
                    oldestVersion: 1, currentVersion: 2,
                ...
                onDownloadFile: async (_url, target) => { f.write(target, body); },
                onRestartPatcher: (path) => { assert.deepEqual(nodeFS.readFileSync(path), body); restarted = true; },
            });
...
test("下载截断即使持续上报进度也只有限重试，不安装坏脚本", async () => {
    await withFixture(async (f) => {
        ...
        await f.run({
            localCheckMode: true, onCheckNetwork: async () => true,
            onFetchRemoteText: async () => JSON.stringify({
                oldestVersion: 1, currentVersion: 3,
                patchScript: { path: "https://patch.invalid/patcher.js", version: 3, size: body.length, sha256: createHash("sha256").update(body).digest("hex") },
            }),
            ...
            onRestartPatcher: () => { restarted = true; },
        });
        ...
        const manifest: IManifestInfo = JSON.parse(nodeFS.readFileSync(f.manifestPath, "utf8"));
        assert.equal(manifest.files.some(info => info.path === "patcher.js"), false);
    }, 0);
});
...
for (const source of ["download", "staged", "missing"] as const) {
    test(`坏脚本恢复撤销待安装记录并重新下载，来源=${source}`, async () => {
        await withFixture(async (f) => {
            ...
            const info: IFileInfo = { path: "patcher.js", version: 3, size: body.length,
                sha256: createHash("sha256").update(body).digest("hex"), redirect };
            f.infos.push(info);
            f.saveManifest();
            ...
            await f.run({ onRestartPatcher: () => { restarts++; } });
            ...
            assert.equal(recovered.files.some(file => file.path === "patcher.js"), false);
            ...
            await f.run({
                localCheckMode: true, onCheckNetwork: async () => true,
                onFetchRemoteText: async () => JSON.stringify({ oldestVersion: 1, currentVersion: 2,
                    patchScript: { ...info, path: "https://patch.invalid/patcher.js" },
                    channels: { common: { patches: [] }, languages: {} } }),
                onDownloadFile: async (_url, path) => { downloads++; f.write(path, body); },
                onRestartPatcher: (path) => { restarts++; assert.deepEqual(nodeFS.readFileSync(path), body); },
            });
...
for (const source of ["download", "staged", "applied"] as const) {
    test(`完整待安装脚本仍可恢复，来源=${source}`, async () => {
        await withFixture(async (f) => {
            ...
            const target = nodePath.join(f.applyPath, "patcher.js");
            const redirect = nodePath.join(f.downloadPath, "patcher.js");
            f.infos.push({ path: "patcher.js", version: 3, size: body.length,
                sha256: createHash("sha256").update(body).digest("hex"), redirect });
            f.saveManifest();
            f.write(source === "download" ? redirect : source === "staged" ? `${target}.patching` : target, body);
            let restarts = 0;
            await f.run({ onRestartPatcher: (path) => { restarts++; assert.deepEqual(nodeFS.readFileSync(path), body); } });
            assert.equal(restarts, 1);
            assert.deepEqual(f.errors, []);
        }, 0);
    });
...
test("补齐包内语言版本后中断，磁盘仍允许旧脚本自更新，完成资源后保留脚本记录", async () => {
    await withFixture(async (f) => {
        ...
        const engine = f.engine({
            localCheckMode: true, onCheckNetwork: async () => true, maxRetryCount: 1,
            onFetchRemoteText: async () => JSON.stringify(remote),
            onDownloadFile: async () => { downloadAttempts++; throw new Error("simulated interruption before resource download"); },
        });
        ...
        f.errors.length = 0;
        await startPatcher({ ...engine,
            onDownloadFile: async (_url, target) => { f.write(target, "zip fixture"); },
            onUnzipFile: async (_zip, target) => { f.write(nodePath.join(target, "file-list.json"), "[]"); },
        }, [EGameLanguage.RU_RU], EGameLanguage.RU_RU);
        assert.deepEqual(f.errors, []);
